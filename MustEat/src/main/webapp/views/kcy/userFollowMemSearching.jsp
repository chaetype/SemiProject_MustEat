<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	import="java.util.ArrayList, com.mz.member.model.vo.Member"%>
<%
...
<body>
	...
	<div class="wrap77">
			...
			<div class="wrap55">
				<div class="input-group mb-3">
					<input type="text" name="findKeyword"
						class="form-control input-text" placeholder="검색할 유저를 입력해주세요"
						width="70%" aria-label="Recipient's username"
		...
		<form action="<%= contextPath %>/insert.fo" method="post">
			<div>
				<table class="tt"
					style="width:100%; background-image: url(<%=contextPath%>/resources/image/cy/retrobg.jpg);">
					<% for(Member m : list){ %>
					<tr>
						<th width="50"><input type="hidden" name="fingMno"
							value="<%=m.getMemNo()%>"></th>
						<td width="70"><img
							src="<%=contextPath%>/resources/image/mz.png"
							style="width: 40px; height: 40px; border-radius: 50px; overflow: hidden;"></td>
						<td width="100"><%=m.getMemNickname() %></td>
						<td width="100"><%=m.getMemId() %></td>
						<td width="100">리뷰 갯수 <%=m.getCstreview() %></td>
						<th width="200"><button type="submit" class="btn1"
								style="padding: 0.3em 1.2em; margin: 0 0.1em 0.1em 0;">follow</button></th>
					</tr>
					<%} %>
				</table>
			</div>
